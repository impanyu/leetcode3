...
## [809. Expressive Words](https://leetcode.com/problems/expressive-word)
1. use a class to represent the run length encoding of string 
...
public: 
    int expressiveWords(string S, vector<string>& words) {
        ...
        for(string word: words){
            RLE WRLE(word);
            if(WRLE.key!=SRLE.key) continue;
            int i=0;
            for(; i<SRLE.counts.size();i++){
                if(SRLE.counts[i]<WRLE.counts[i] || SRLE.counts[i]<3 && WRLE.counts[i]!=SRLE.counts[i]) 
                    break;
            }
            if(i==SRLE.counts.size()) ans++;            
        }
        return ans;
    }
};
